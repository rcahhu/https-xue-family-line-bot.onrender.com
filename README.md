...
這是一個可先跑起來的 MVP：LINE Messaging API webhook + LIFF 旅行日記頁面。使用者可以建立一本旅行日記，例如「宜蘭」，再管理行程、是否購票、是否訂位、價格、同行成員邀請與旅伴許願。

## 已做好的功能

- LINE 指令：
  - 加入好友或輸入 `功能`：直接顯示 Flex 資料方塊，不再只用文字指令教學
  - 功能方塊包含打開日記、建立旅遊日記、邀請家人、小幫手、問旅遊助理
  ...
  - `開啟`：取得目前旅行日記連結
- AI 旅遊助理：
  - 有設定 `OPENAI_API_KEY` 時，`問 ...` 和 `目的地推薦` 會直接在 LINE 聊天室呼叫 OpenAI 回答
...
- LIFF 頁面：
  ...
  - 同伴頁可看同伴人數與名單
  - 旅伴許願池，支援想吃、想去、想玩、其他
  - 同行成員頁可邀請家人加入日記本，也可手動新增未使用 LINE 的同行者
...
或按功能方塊的 `目的地推薦`，就會在聊天室直接得到 AI 回答。OpenAI API 會依你的 OpenAI 帳號用量計費；不想用網路搜尋時，把 `OPENAI_ENABLE_WEB_SEARCH` 改成 `false`。
...
- 接 Google Places、觀光署資料或自家後台，讓「目的地推薦」變成即時搜尋。
